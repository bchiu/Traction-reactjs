...
/* helper methods ************************************************************/
    ...
    this.sliceInt16 = function(buf, scale, index) {
        var res = buf[index] << 8 | buf[index+1];
        return res/scale;
    }

    this.sliceInt32 = function(buf, scale, index) {
...
/* event handlers ************************************************************/
    ...
    this.onDataReady = function(buf) {
        var packet = Array.prototype.slice.call(new Uint8Array(buf));
        this.deserialize(packet);
    }.bind(this)

    this.onDataError = function(error) {
        console.log(error);
    }

/* public methods ************************************************************/

    this.bindData = function(dataToBind) {
        this.data = dataToBind;
    }

    this.send = function(payload) {
        if (!this.listening) return;
        var packet = this.serialize(payload);
    ...
    this.startListening = function() {
        this.listening = true;
        bluetoothSerial.subscribeRawData(this.onDataReady, this.onDataError);
