 ...
 JustGage = function(config) {
  ...
  obj.canvas.customAttributes.pki = function (value, min, max, w, h, dx, dy, gws, donut) {
    ...
    if (donut) {
      ...
      Ro = w / 2 - w / 7;                     // radius outer circle
      Ri = Ro - w / 6.666666666666667 * gws;  // radius inner circle

      Cx = w / 2 + dx;      // center x
      Cy = h / 1.95 + dy;   // center y

      Xo = w / 2 + dx + Ro * Math.cos(alpha);     // center x outer circle
      Yo = h - (h - Cy) - Ro * Math.sin(alpha);   // center y outer circle
      Xi = w / 2 + dx + Ri * Math.cos(alpha);     // center x inner circle
      Yi = h - (h - Cy) - Ri * Math.sin(alpha);   // center y inner circle

      path = "M" + (Cx - Ri) + "," + Cy + " ";    // absolute move to
      path += "L" + (Cx - Ro) + "," + Cy + " ";   // absolute line to

      // arc path: "A rx ry x-axis-rotation large-arc-flag sweep-flag x y"

      // if value is past halfway point, draw first half arc
      if (value > ((max - min) / 2)) {
        path += "A" + Ro + "," + Ro + " 0 0 1 " + (Cx + Ro) + "," + Cy + " "; // outer circle path 
      }

      path += "A" + Ro + "," + Ro + " 0 0 1 " + Xo + "," + Yo + " ";          // outer circle path
      path += "L" + Xi + "," + Yi + " ";                                       

      if (value > ((max - min) / 2)) {
        path += "A" + Ri + "," + Ri + " 0 0 0 " + (Cx + Ri) + "," + Cy + " "; // inner circle path
      }
...
JustGage.prototype.refresh = function(val, max) {
  ...
  // flip gauge for negative range
  if (neg) {
    obj.level.transform("s1,-1");
  }
